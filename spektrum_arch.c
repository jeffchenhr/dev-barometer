...
#include "radio_control.h"	//modified/added all thses
#include "spektrum_arch.h"
#include "uart.h"
#include "gpio.h"
#include "lisa_m_2.0.h"
#include "lisa_m_common.h"

//#include BOARD_CONFIG 	//for future reference, this means lisa_m_2.0.h
//#define STM32F1 TRUE		//made for us, set previously by makefile...
#define SPEKTRUM_CHANNELS_PER_FRAME 7
#define MAX_SPEKTRUM_FRAMES 2
#define MAX_SPEKTRUM_CHANNELS 16
#define MAX_PPRZ 9600		//I believe this represents the max value from the radio
...
#define RADIO_CONTROL_SPEKTRUM_PRIMARY_PORT SPEKTRUM_UART1		//custom
#define RADIO_CONTROL_SPEKTRUM_SECONDARY_PORT SPEKTRUM_UART5	//custom
  ...
  if(PrimarySpektrumState.RcAvailable) {
    ...
    if (ChannelCnt >= (MaxChannelNum + 1)) {
      ...
      for (int i = 0; i < (MaxChannelNum + 1); i++) {
        ...
        /*if (i == RADIO_ROLL ) { // ADDED BY MD TO WORK WITH BROKEN TRANSMITTER!!
          radio_control.values[i] += 5292;
          if (radio_control.values[i] < 0) radio_control.values[i] = (9 * radio_control.values[i]) / 4;
          if (radio_control.values[i] > 0) radio_control.values[i] = (6 * radio_control.values[i]) / 4;
          if (radio_control.values[i] < -MAX_PPRZ) radio_control.values[i] = -MAX_PPRZ;
          if (radio_control.values[i] >  MAX_PPRZ) radio_control.values[i] =  MAX_PPRZ;
        }*/
...
void SecondaryUart(_ISR)(void) {
  ...
  //uint32_t tmpc = (USART_CR1(SecondaryUart(_DEV)) & USART_CR1_TXEIE); // TXEIE: TXE interrupt enable
  //uint32_t tmpd = (USART_SR(SecondaryUart(_DEV)) & USART_SR_TXE); // TXE: Transmit data buffer empty
  ...
  /*uint32_t fuckyouuart = USART_SR(SecondaryUart(_DEV));
  if (fuckyouuart & USART_SR_ORE) {
	  //USART_SR(SecondaryUart(_DEV)) &= ~USART_SR_ORE;
	  uint32_t arg1 = USART_SR(SecondaryUart(_DEV));
	  uint32_t arg2 = USART_DR(SecondaryUart(_DEV));
  }*/
  //uint32_t tmpa = (USART_CR1(SecondaryUart(_DEV)) & USART_CR1_RXNEIE); // RXNEIE: RXNE interrupt enable
  //uint32_t tmpb = (USART_SR(SecondaryUart(_DEV)) & USART_SR_RXNE); // RXNE: Read data register not empty
  if (((USART_CR1(SecondaryUart(_DEV)) & USART_CR1_RXNEIE) != 0) &&
	...
	//uint8_t b = USART_DR(SecondaryUart(_DEV));
    SpektrumParser(b, &SecondarySpektrumState, TRUE);
  } else {
	  // check for the lovely overrun error...
	  if (USART_SR(SecondaryUart(_DEV)) & USART_SR_ORE) {
		  uint8_t b = USART_DR(SecondaryUart(_DEV));
	  }
  }
