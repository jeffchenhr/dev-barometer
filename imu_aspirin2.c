...
gpio_clear(GPIOA, GPIO8); // 0
gpio_clear(GPIOB, GPIO4); // 1
gpio_clear(GPIOC, GPIO2); // 2
gpio_clear(GPIOC, GPIO5); // 3
gpio_clear(GPIOC, GPIO15); // 4*/
...
PRINT_CONFIG_MSG("Reading MPU slaves")
  ...
  // 75 Hz (max in continuous measurement mode), normal measurement mode, 1 sample per output (averaging)
  ...
  // +/- 1.3 Ga = 1090 LSb/Gauss = 0.92 mG/LSb
  mpu_set( MPU60X0_REG_I2C_SLV4_ADDR, (HMC58XX_ADDR >> 1));
  ...
  // continuous measurement mode, normal speed I2C
  mpu_set( MPU60X0_REG_I2C_SLV4_ADDR, (HMC58XX_ADDR >> 1));
