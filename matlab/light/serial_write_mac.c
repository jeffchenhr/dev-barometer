...
#define PKT_SIZE (54+6)
#define PKT_COUNT 1024
#define RETURN_SIZE (25+3)
#define RETURN_SIZE_STATUS 10
...
int readSerialIMU(double *dataarray, double *statusarray, int ndim) {
    ...
    if (count >= ndim) {
        ...
        for (idim = 0; idim < ndim; idim++) {
            ...
            for (i = 0; i < 24+3; i++) { // 1 (tick) + 9 (mag, gyro, acc) + 6 (actuator) + 8 (2x quat) +3 pos
                val = buff[ptr_out][2*i+1] * 256 + buff[ptr_out][2*i];
                if (i > 0) {
                    if (val > 32767) val = val - 65536;
                }
                data[i] = (double)val;
            ...
            status[4]  = (statval >>  9) & 0x01; // posctrl
            status[8]  = (statval >> 10) & 0x01; // elev d/r
            status[9]  = (statval >> 11) & 0x01; // flap
            status[5]  = (statval >> 12) & 0x0f; // vicon
                        

            // normalize quaternions
            for (i = 0; i < 8; i++) {
                data[16 + i] = data[16 + i] / 32768.0;
            }
            
            // normalize positions
            for (i = 0; i < 3; i++) {
                data[24 + i] = data[24 + i] / 1000.0;
            }
