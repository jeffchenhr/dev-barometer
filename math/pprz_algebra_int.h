...
// MD
#define INT32_VECT3_BOUND_NORM(vo, vi, nmax) {		        		\
    int32_t n2 = (vi).x*(vi).x + (vi).y*(vi).y + (vi).z*(vi).z;   	\
    if (n2 > nmax*nmax) {											\
    	int32_t n;													\
    	INT32_SQRT(n, n2);											\
    	(vo).x = (vi).x * nmax / n;									\
...
#define INT32_QUAT_VEC_NORM(n, q) {			                        \
    int32_t n2 = (q).qx*(q).qx + (q).qy*(q).qy + (q).qz*(q).qz; \
    INT32_SQRT(n, n2);							\
  }
...
#define INT32_EXPCOORDS_OF_QUAT(_e, _q) {					\
									\
    const int32_t qx2  = INT_MULT_RSHIFT((_q).qx,(_q).qx, INT32_QUAT_FRAC); \
    const int32_t qy2  = INT_MULT_RSHIFT((_q).qy,(_q).qy, INT32_QUAT_FRAC); \
    const int32_t qz2  = INT_MULT_RSHIFT((_q).qz,(_q).qz, INT32_QUAT_FRAC); \
...
#define INT32_EULERS_OF_RMAT(_e, _rm) {					\
    ...
    const float theta = -asinf( dcm02 );				\
    const float psi   = atan2f( dcm01, dcm00 );				\
    (_e).phi   = ANGLE_BFP_OF_REAL(theta);				\
    (_e).theta = ANGLE_BFP_OF_REAL(phi);				\
    (_e).psi   = ANGLE_BFP_OF_REAL(-psi);				\
    									\
  }
